...
Formula reference — validated in 03_metric_validation.ipynb §3.6:
  ...
  Feature scaling (0 → 1):
    ...
    timing_clean = 1.0 - clip(timing_delta / TIMING_NORMALISER_LAPS, 0, 1)

  Weighted aggregate:
    raw = PTL_clean * 0.20 + PPD_clean * 0.5905 + timing_clean * 0.2095

  Final scale — min-max stretch per session to full 0–100 range:
    UTS = (raw - season_min) / (season_max - season_min) * 100

  UTS = 0   → Critical Risk (worst stop in the season)
  UTS = 100 → Optimal Safety (best stop in the season)

Key differences from PRD v2 defaults:
  - Scale is 0–100, not -100 to +100
  - Weights are Ridge-regression fitted, not assumed
  - Min-max stretch applied at season level, not per-stop clip
...
def compute_uts_raw(
    ...
    stretch across the full season to produce the final 0–100 UTS.
    """
    if weights is None:
        weights = UTS_WEIGHTS
    ...
    timing_clean = float(
        1.0 - np.clip(timing_delta / TIMING_NORMALISER_LAPS, 0.0, 1.0)
    )
...
def apply_minmax_stretch(raw_scores: pd.Series) -> pd.Series:
    ...
    min-max normalisation across the full season.
...
def classify_strategy(ptl: float) -> str:
    """
    Classify a stop's strategy type based on PTL.
...
def compute_uts_for_session(
    ...
    Steps:
      ...
      2. Adjust pit loss for SC/VSC stops
      3. Compute PTL for every stop
      4. Compute raw UTS score for scoreable stops (green flag, non-tactical)
      5. Apply min-max stretch across scoreable stops → final 0–100 UTS
      6. Classify strategy type
      7. Mark is_opportunistic (SC stop that gained position)
...
def _log_session_summary(df: pd.DataFrame) -> None:
    ...
    if scored.empty:
        return
    logger.info(
        "UTS summary — %d scored | mean=%.1f | reactive=%d proactive=%d neutral=%d | excluded=%d",
        len(scored),
        scored["uts"].mean(),
